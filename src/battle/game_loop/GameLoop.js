let GameLoop = cc.Class.extend({
    ...
    update : function() {
        ...
        for (let i = 0; i < tickAction.length; i++) {
            this.fromEventToGameAction(tickAction[i])
        }
    ...
    fromEventToGameAction : function(userEvent) {
        switch (userEvent.getType()) {
            case UserEvent.Type.CREATE_MONSTER:
                // TODO : Thực hiện thả quái với thông tin đầu vào
                cc.log("CREATE MONSTER")
                this.getMonsterController().createMonster(MonsterConfig.Type.CROW_SKELETON)
                break
